...
function train(pokemon, candy) {
  ...
  if (pokemon['evolutionLine'] !== undefined) {
    for (var i=0; i<pokemon['evolutionLine'].length; i++) {
      if(level >= pokemon['evolutionLine'][i]['level']){
        evolusi.push(pokemon['evolutionLine'][i]['name']);
      }
    }    
    return `Congratulations, your Charmander grew to LV.${level}! And it evolved into ${evolusi}!`;
  }else {
    return `Congratulations, your Charmander grew to LV.${level}!`;
  }
}
